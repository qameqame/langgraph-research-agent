...
def _rule_based_route(state: State) -> str:
    ...
    注意: 現時点(TODO未実装)ではResearcher/Writerの2択に対応している。
    TODO1〜6でFactCheckerを追加したら、Writerの後に
    `if not any(c.startswith("[FactChecker]") for c in contents): return "FactChecker"`
    を挟むように、このフォールバックも拡張すること。"""
...
#         (ローカルモデルは長い会話履歴を渡されると空/短すぎる応答を返すことが
#         あるので、writer_nodeと同様にMIN_FACTCHECK_CHARSで検証して再試行する
#         防御を入れるとより安定します)
#
